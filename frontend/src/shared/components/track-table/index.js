...
const TrackTable = (props) => {
      ...
      <SubMenu title="Add to Playlist">
        {props.session.playlists.map((playlist, index) => {
          return (
            <Menu.Item
              onClick={() => {
                addTrackToPlaylist(id, playlist.id);
              }}
            >
              {playlist.title}
            </Menu.Item>
  ...
  let columns = [
          ...
          props.player.currentTrack === key ? (
            <>
              {props.player.audioPlaying === true ? (
                <PauseCircleOutlined
                  onClick={dispatchPause.bind(this)}
                  style={{ fontSize: 24 }}
                />
              ) : (
                <PlayCircleOutlined
                  onClick={dispatchResume.bind(this)}
            ...
            <PlayCircleOutlined
              onClick={onSetTrack.bind(this, key)}
              className="show-on-hover"
              style={{ fontSize: 24 }}
            />
          )}
    ...
    },
    {
      ...
      key: "credits",
      render: (credits) => (
        <Popover content={creditContent(credits)} title="Credits">
  ...
  if (props.session.loggedIn && props.options === true) {
    columns.push({
      ...
      render: (id) => (
        <Dropdown
          trigger="click"
          overlay={props.menu ? props.menu(id) : menu.bind(this, id)}
        >
          <Button shape="circle">
            <EllipsisOutlined />
          </Button>
